...
#                 parent_cat = ingredient_dictionary[each][2]
...
#             not_in_sub_category_list = [each for each in ingredient_list if each.lower() not in ingredient_dictionary.keys()]

#             sub_category_list = [ingredient_dictionary[each.lower()][0]
#                                  for each in ingredient_list if each.lower() in ingredient_dictionary.keys()]

#             parent_category_list = [ingredient_dictionary[each.lower()][1] 
#                                     for each in ingredient_list if each.lower() in ingredient_dictionary.keys()]
...
#         for i, j, k in zip(top_5_df_merged['CO2 Emission Percentile'],
#                            top_5_df_merged['multiplier'], top_5_df_merged['parent_category']):
...
#         dict_df = pd.DataFrame(zip(ingredient_dictionary.keys(), 
#                                    [i[0] for i in ingredient_dictionary.values()], 
#                                    [i[1] for i in ingredient_dictionary.values()]),
...
# def lifestage_calc(species: str, age_in_months: int, breed: Union[str, None] = None) -> str:
#     """
#     Calculates the life stage of a pet based on species, age in months, and optionally breed (for dogs).

#     Args:
#     - species (str): The species, either 'Cat' or 'Dog'.
#     - age_in_months (int): Age of the pet in months.
#     - breed (str, optional): The breed of the dog (required only if species is 'Dog').

#     Returns:
#     - str: The life stage of the pet: 'Kitten', 'Adult', 'Senior', or 'Puppy' based on the inputs.
#     """
#     if species == 'Cat':
#         if age_in_months <= 12:
#             lifestage = 'Kitten'
...
#             lifestage = 'Senior'
#     else:
#         raise ValueError("Please provide the dog's breed for age calculation.")
#     return lifestage

# def req(species: str, lifestage: str, weight: float, age_in_months: int = None, condition: str = None) -> dict:
#     """
#     Calculates recommended daily intake of calories, protein, and fat based on
#     species, life stage, weight, and optionally age (for puppies) and condition.

#     Args:
#     - species (str): The species, either 'Cat' or 'Dog'.
#     - lifestage (str): The life stage, e.g., 'Kitten', 'Puppy', 'Adult', 'Senior'.
#     - weight (float): Weight of the pet in kilograms.
#     - age_in_months (int, optional): Age of the pet in months (for puppies).
#     - condition (str, optional): The pet's condition, if applicable.

#     Returns:
#     - dict: A dictionary containing the recommended daily intake:
#         - 'Calories': Recommended daily calories.
#         - 'Protein': Recommended daily protein intake.
#         - 'Fat': Recommended daily fat intake.
#     """
...
#         if age_in_months and age_in_months <= 4:
#             kcal_per_day = 3 * rer
#             protein_per_day = (45/1000) * kcal_per_day
#             fat_per_day = (kcal_per_day * 21.3) / 1000

#         elif age_in_months and age_in_months > 4:
#             kcal_per_day = 2 * rer
#             protein_per_day = (35/1000) * kcal_per_day
#             fat_per_day = (kcal_per_day * 21.3) / 1000
...
#     print({"Calories": round(kcal_per_day, 2),
#             "Protein": round(protein_per_day, 2),
#             "Fat": round(fat_per_day, 2)})
