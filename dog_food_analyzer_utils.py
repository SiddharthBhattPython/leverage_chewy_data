...
def preprocess_ingredients(ing_str):
    ...
    Returns: Nested Dictionary containing the name, category, quality and type information of the ingredient.
    '''
    try:
        ...
        for k in mapping_dict.keys():
            final_sorted_dict[k] = new_sorted_dict[k]
...
#             rule="NA"
        ...
        # Start assigning proportions using an exponential decay approach
        for i in range(n):
            # Apply exponential decay for proportion calculation
            proportions[ingredients[i]] = 100 * (math.exp(-0.07 * i) if i < n * 0.20 else math.exp(-0.3 * (i - n * 0.20)))  # Adjust the decay rate as needed
        ...
        if total_assigned > 0:
            scale_factor = 100 / total_assigned
            for ingredient in proportions:
                proportions[ingredient] *= scale_factor

        # Ensure all proportions are non-zero and strictly decreasing
        for i in range(1, n):
...
#             proportions[ingredient] = round(proportions[ingredient], 2)
        ...
        for each in return_dict.items():
            e_dict[each[0]] = [each[1]['Ingredient'], energy_contributions[each[1]['Type']]["Energy from Fat (kcal)"]*(each[1]['Proportion']/100),
                               energy_contributions[each[1]['Type']]["Energy from Protein (kcal)"]*(each[1]['Proportion']/100),
                               energy_contributions[each[1]['Type']]["Energy from Carbs (kcal)"]*(each[1]['Proportion']/100),
                               energy_contributions[each[1]['Type']]["Total Energy (kcal)"]*(each[1]['Proportion']/100), each[1]["Proportion"]]
...
#                                ascending=False)
        ...
        total_energy = e_df['Total KCAL'].sum()
        
        e_df['Emissions'] = [(return_dict[each]['Proportion']/100)*return_dict[each]['CO2'] for each in e_df['ing']]
        e_df['Emissions'] = [float(each*kcal/1000) for kcal, each in zip(e_df["Total KCAL"], e_df['Emissions'])]
...
#             em_dict[each[0]] = each
        ...
        co2_index = 0
        print(err)
        e_df = {"Enter Ingredients Properly!"}
        # Write the dictionary to a JSON file 
#         with open('json_output.json', 'w') as f:
#         # saving dictionary into json file
