...
#             proportions[ingredient] = round(proportions[ingredient], 2)
        ...
        for each in return_dict.items():
            e_dict[each[0]] = [each[1]['Ingredient'], energy_contributions[each[1]['Type']]['dry']*(each[1]['Proportion']/100), each[1]["Proportion"]]
...
#                                ascending=False)
        ...
        total_energy = e_df['KCAL'].sum()
        
        e_df['Emissions'] = [(return_dict[each]['Proportion']/100)*return_dict[each]['CO2'] for each in e_df['ing']]
        e_df['Emissions'] = [float(each*kcal/1000) for kcal, each in zip(e_df["KCAL"], e_df['Emissions'])]
